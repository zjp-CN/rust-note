...
而目前唯一稳定的隐式提升形式就是生命周期延长。一个重要事实是 `const fn` 必须在具有 `#[rustc_promotable]`
属性的情况下才具备隐式提升的资格，（见 [有资格的引用不一定被自动提升] 一节）。

此外，隐式提升的一个前提是，表达式必须不能求值失败。这方面有诸多限制，首先需要区分哪些操作一定不会失败，哪些操作可能导致失败（这在
RFC 中具体罗列了，这里不重复列了）。

最终的部分结果是：
1. （自定义） `const fn` 因不具有 `#[rustc_promotable]` 而不具备隐式提升的资格，如果延长生命周期，必须放入常量中明确要求编译期求值
2. 除法[^Division]、取模、索引操作不具备隐式提升的资格，如果延长生命周期，必须放入常量中明确要求编译期求值
